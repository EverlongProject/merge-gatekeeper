...
If you want to exclude CodeQL or other GitHub Advanced Security checks from merge gating, use `ignored`. Those checks do not always have a backing workflow run in the Actions workflow-runs API, so `ignore-dynamic-github-workflows` does not exclude them.
