# League Fork of Merge Gatekeeper
...
| `required` | Jobs that must appear before Merge Gatekeeper can succeed. Once observed, they follow normal job semantics. Defined as a comma-separated list. This stays inside Merge Gatekeeper, so GitHub branch protection still only needs `merge-gatekeeper` as the required check.                                    |          |
